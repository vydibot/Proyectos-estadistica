...
# Configuración global para el documento HTML
...
# Crear carpeta para guardar todos los resultados
dir.create("resultados taller 1", showWarnings = FALSE)

# 1. Carga de datos con parámetros explícitos (sep para comas de archivo, dec para decimales)
datos <- read.csv("data/EDUCACION_LIMPIO.csv", sep = ",", dec = ".")
...
write.csv(resumen, "resultados taller 1/Resumen_Estadistico.csv", row.names = FALSE)
...
print(p_hist)
ggsave("resultados taller 1/Histogramas.png", plot = p_hist, width = 12, height = 10)
```
...
# Mostrar y guardar gráfico
print(p_box)
ggsave("resultados taller 1/Boxplots.png", plot = p_box, width = 8, height = 10)
```
...
for (i in 1:length(pares)) {
  ...
  if(var1 %in% names(datos) & var2 %in% names(datos)) {
    ...
    print(p)
    ggsave(paste0("resultados taller 1/Dispersion_", var1, "_vs_", var2, ".png"), plot = p, width = 6, height = 4)
  }
}
```
...
write.csv(matriz_corr, "resultados taller 1/Matriz_Correlacion.csv")
...
# Mostrar y guardar gráfico
print(p_corr)
ggsave("resultados taller 1/Heatmap_Correlacion.png", plot = p_corr, width = 10, height = 10)
```
